...
export default function HrPerformanceReport() {
  ...
  const getHrPerformanceData = async (startDate, endDate, downliners) => {
    ...
    try {
      const response = await hrPerformanceReport(payload);
      console.log("hr performance report response", response);
      setReportData(response?.data?.data?.month_wise || []);
      setTotalCounts(response?.data?.data?.totals || null);
    } catch (error) {
      setReportData([]);
      setTotalCounts(null);
      console.log("hr performance report error", error);
    } finally {
      setTimeout(() => {
        setLoading(false);
      }, 300);
