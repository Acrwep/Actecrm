...
export default function PostSalePerformanceReport() {
  ...
  const getPostSalePerformanceData = async (startDate, endDate, downliners) => {
    setLoading(true);
    const payload = {
      user_ids: downliners,
    ...
    try {
      const response = await postSalePerformanceReport(payload);
      console.log("post sale performance report response", response);
